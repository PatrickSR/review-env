...
### 需求:容器停止与清理
Review Service 必须支持三种方式停止并移除容器：Web 页面手动停止、超时自动清理、MR 状态变更触发。
...
### 需求:状态恢复
Review Service 重启后必须从 SQLite 数据库恢复容器管理状态，并与 Docker 实际状态校验。

#### 场景:服务重启
- **当** Review Service 进程重启
- **那么** 服务从 containers 表读取记录，对每个记录检查 Docker 容器是否实际存在，删除已不存在的记录，更新端口映射信息
