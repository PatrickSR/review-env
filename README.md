...
MR 临时开发环境服务。为 GitLab Merge Request 提供一键式 Review 环境——Reviewer 通过 URL 即可进入已就绪的 Web IDE（code-server），内置 AI 编码工具，支持查看 UI、交互追问、即时修改代码。

## 功能概览

- 多项目管理：通过 SPA 管理界面配置多个 GitLab 项目，每个项目独立的 PAT、Webhook Secret、镜像配置
- MR 自动感知：GitLab Webhook 自动检测 MR 事件，MR 打开时自动发布 Review 环境链接
- 一键启动容器：在控制面板选择 AI 工具镜像，一键创建 Review 容器
- Web IDE：容器内运行 code-server（浏览器版 VS Code），通过 Docker 随机端口映射直接访问
- 应用预览：容器内 dev server 端口自动映射到宿主机随机端口，直接通过端口访问
- 自定义镜像构建：在管理界面编写 Dockerfile，基于 review-base 基础镜像在线构建 Review 镜像
...
│                        用户浏览器                            │
├──────────────────────┬──────────────────────────────────────┤
│   React SPA          │   code-server / dev server           │
│   (管理界面)         │   (直接访问容器随机端口)              │
...
│   │       │   ├── ImageBuild.tsx      # 镜像构建（Dockerfile 编辑器）
│   │       │   ├── Containers.tsx      # 容器监控
│   │       │   └── Terminal.tsx        # MR Review 控制面板
...
├── docker/
│   ├── Dockerfile.base         # review-base 基础镜像
│   └── entrypoint.sh           # 容器入口脚本（clone → before_script → code-server）
...
1. 编写或粘贴 Dockerfile（默认预填基于 `review-base` 的示例）
2. 填写镜像名称和 Tag
3. 点击构建，实时查看构建日志
...
- 环境变量：JSON 格式的额外环境变量
- Before Script：容器启动并 clone 代码后执行的初始化脚本（如 `npm install`）
...
1. 选择 AI 工具镜像
2. 等待容器初始化（clone 代码 → 执行 before_script → 就绪）
3. 点击「打开 IDE」在新标签页进入 code-server
...
├── env_vars (JSON), ports (逗号分隔), before_script
└── sort_order, enabled
...
- 端口策略：code-server (8080) + 用户配置的额外端口，全部映射到宿主机随机端口
...
用户在 Web 界面编写 Dockerfile，通过 Docker API 直接传入 tar 流构建，不依赖本地文件。推荐基于 `review-base` 基础镜像（包含 git、code-server、curl、entrypoint.sh）扩展，安装所需的语言运行时和 AI 工具。

`review-base` 基础镜像的 entrypoint 流程：
- 配置 git 凭证 → shallow clone MR 分支 → 执行 before_script（如有）→ 启动 code-server
- before_script 通过 `BEFORE_SCRIPT` 环境变量传入（base64 编码），在 clone 后、code-server 启动前执行
- 任何步骤失败时写 error 状态但 code-server 照样启动，确保用户能进入环境排查问题
